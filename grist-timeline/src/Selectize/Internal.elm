module Selectize.Internal exposing
    ( Action(..)
    , Entry
    , Heights
    , Input
...
update :
    ...
    -> State a
    -> Msg a
    -> ( State a, Cmd (Msg a), Action a )
update maybeSelection state msg =
    case msg of
        NoOp ->
            ( state, Cmd.none, NoAction )
        ...
        SelectKeyboardFocusAndBlur ->
            ( state |> reset
            , blur state.id
            , ChangeSelection True (state.zipList |> Maybe.andThen currentEntry)
            )

        ClearSelection ->
            ( state
            , Cmd.none
            , ChangeSelection True Nothing
            )
    ...
    let
        ...
        menuAttrs =
            ...
            , Attributes.style "position" "absolute"
                ...
                ++ noOp (config.menu (state.zipList /= Nothing))
    in
    case state.zipList of
        Nothing ->
            Html.div
